...
const SECTION_BLUEPRINTS = {
  0: {
    ...
    description: "One-page synthesis of affiliate program health, strategic context, and priority interventions for executive decision-makers.",
    scr: {
      situation: "State the affiliate program scale: total GMV, active publisher count and ratio, and aggregate commission structure. Anchor every claim to the supplied metrics.",
      complication: "Surface the critical structural gaps that create near-term risk: top-heavy revenue concentration, activation deficit vs 40-60% industry floor, publisher type coverage gaps, approval rate exposure. Quantify each risk precisely.",
      resolution: "Prescribe the top 3 highest-leverage interventions with specific, quantified targets. Frame as a 30/60/90-day operating sequence. Each action must tie to a specific metric and an owner role.",
    },
    required: [
      "Single bottom-line verdict: Is this program structurally healthy, at-risk, or in distress? Use precise language with data. No hedging.",
      "Benchmark every KPI against named industry standards: active ratio healthy range 40-60%, top-10 GMV concentration healthy <50%, approval rate healthy >85%, GMV/active publisher minimum threshold.",
      "For each KPI below benchmark, quantify the gap and state the cost or risk in GMV terms.",
      "Name the single most critical risk and the single most valuable opportunity in the current dataset.",
      "End with a 30/60/90 action matrix: what to do, who owns it, what KPI it moves.",
    ],
    voice: "Lead with the conclusion. Use pyramid principle: answer first, then support. Write for a CFO who has 90 seconds.",
  },
  1: {
    title: "Activation Funnel",
    description: "Rigorous analysis of publisher activation rate, dormant segment sizing, and evidence-based reactivation levers.",
    scr: {
      situation: "State total publisher count, active count, and active ratio. Segment the inactive base by potential value (clicked but not converted, no clicks, brand-new).",
      complication: "If active ratio is below 40%, the program has a structural productivity problem. Quantify the GMV opportunity locked in the dormant base using average GMV/active publisher as a multiplier.",
      resolution: "Prescribe a tiered reactivation playbook: immediate outreach targets (highest potential inactive), creative refresh requirements, program structure changes (commission floor, onboarding overhaul).",
    },
    required: [
      "Active ratio with precise benchmark: below 30% = critical, 30-40% = at-risk, 40-60% = healthy, above 60% = strong.",
      "Dormant base opportunity: estimate potential GMV uplift if inactive-to-active conversion improves by 10 percentage points.",
      "Segment inactive publishers by type (deal, content, social, search) to prioritize outreach.",
      "Identify the top 10 inactive publishers by historical or click volume as reactivation targets.",
      "Reactivation tactics mapped to specific publisher types: content partners need briefs, deal partners need codes, social partners need exclusives.",
    ],
    voice: "Be specific about which partners to reactivate first and why. Avoid generic 'email all dormant partners' recommendations.",
  },
  2: {
    title: "Revenue Concentration Analysis",
    description: "Pareto curve analysis of GMV fragility, top-partner dependency risk, and a structured diversification roadmap.",
    scr: {
      situation: "Plot the Pareto distribution: how many publishers drive 50%, 80%, and 100% of GMV. State top-1, top-3, and top-10 concentration ratios precisely.",
      complication: "If top-10 concentration exceeds 50%, the program has dangerous single-point-of-failure risk. If top-1 exceeds 20%, any relationship deterioration would materially harm GMV. Quantify the GMV at risk if top partners churn.",
      resolution: "Identify specific mid-tier partners (rank 11-30) with scale potential. Define a concentration reduction roadmap with target metrics at 90-day and 180-day horizons.",
    },
    required: [
      "Pareto analysis: publishers needed for 50% GMV (benchmark: healthy >= 10, at-risk 5-10, critical < 5).",
      "GMV-at-risk calculation: if top-3 partners churned tomorrow, what % of program GMV would be lost?",
      "Name the top 5 over-indexed partners by GMV share and flag any single partner above 15% threshold.",
      "Identify 5-10 mid-tier partners with the highest GMV/commission efficiency as scale candidates.",
      "Diversification target: recommend a specific top-10 share ceiling (e.g., reduce from X% to 45%) and timeline.",
    ],
    voice: "Use the Pareto data from the evidence tables. Never invent concentration numbers — they must match the supplied metrics.",
  },
  3: {
    title: "Mix Health — Publisher Type Distribution",
    description: "Structural audit of publisher type coverage, imbalance diagnosis, and recruitment priorities to optimize the partner portfolio.",
    scr: {
      situation: "Map current GMV and publisher count by type: content/editorial, deal/coupon, cashback/loyalty, social/video, comparison, search. State what the current distribution is.",
      complication: "A healthy affiliate mix relies on multiple content types to cover the full purchase funnel. Identify which types are over-represented (creating redundancy risk) and which are under-represented (creating coverage gaps).",
      resolution: "Prescribe a recruitment roadmap targeting specific underrepresented publisher types. Match each type to a specific funnel stage and GMV growth lever.",
    },
    required: [
      "Type-by-type breakdown: GMV share, publisher count, and active ratio for each publisher type.",
      "Ideal portfolio target: benchmark against category norms (e.g., content/editorial: 30-40%, deal/coupon: 25-35%, social/video: 15-25%, comparison: 10-20%).",
      "Identify the single most under-represented type that would have the highest GMV impact if recruited.",
      "Name 3-5 specific publisher recruitment targets by type with reasoning (e.g., 'Need a dedicated travel bag comparison site').",
      "Flag any type that is over-indexed and creating unhealthy dependency (e.g., 60%+ coupon = race-to-bottom risk).",
    ],
    voice: "Speak to portfolio construction, not just counting types. Frame mix imbalance as a strategic risk to long-term program economics.",
  },
  4: {
    title: "Efficiency Quadrant",
    description: "2x2 efficiency matrix of publisher performance — scale proven winners, fix cost-inefficient partners, cull dead weight.",
    scr: {
      situation: "Plot publishers on two axes: GMV contribution (high/low) and cost efficiency (CPA or commission rate relative to revenue generated). Four quadrants emerge: Scale, Monitor, Fix, Cull.",
      complication: "Programs with high commission spend on low-output partners are destroying ROI. Identify the specific partners in each quadrant to quantify misallocated spend and opportunity cost.",
      resolution: "Quadrant-specific action playbook: scale high-efficiency high-GMV partners aggressively, fix high-cost low-output partners with new deal structures, cull persistently inefficient partners.",
    },
    required: [
      "Efficiency quadrant definitions with thresholds based on actual data: above-median GMV vs below-median, above-median CPA vs below-median.",
      "Count and aggregate GMV/commission for each quadrant.",
      "Name specific partners in each quadrant (at least top 5 per quadrant if data permits).",
      "ROI improvement estimate: how much commission spend could be reallocated from Cull to Scale quadrant?",
      "Recommended commission restructuring for Fix-quadrant partners: performance tiers, CPA caps, GMV minimums.",
    ],
    voice: "Quantify the efficiency opportunity in dollar terms. 'Moving X partners from Fix to Scale could yield $Yk in GMV with no additional commission spend' is the right frame.",
  },
  5: {
    title: "Approval & Transaction Quality",
    description: "Deep dive into transaction health — approval rate, decline root cause, pending exposure, and governance implications.",
    scr: {
      situation: "State overall approval rate, total approved GMV, pending GMV, and declined GMV. Identify which publishers have anomalous decline or pending rates.",
      complication: "Approval rate below 85% signals systemic issues: fraud, attribution errors, product return rates, or partner non-compliance. Pending GMV above 5% of total creates cash flow uncertainty. Quantify the financial exposure.",
      resolution: "Root-cause framework for decline patterns. Publisher-level compliance audit targets. Process changes to reduce pending exposure and improve attribution quality.",
    },
    required: [
      "Approval rate vs benchmark: 95%+ = strong, 85-95% = acceptable, below 85% = investigate immediately.",
      "Per-publisher approval rate breakdown — identify outliers more than 10 percentage points below overall approval rate.",
      "Declined GMV root cause hypothesis: fraud, returns, T&C violations, attribution errors.",
      "Pending GMV aging: if pending represents a significant share, flag it as a working capital risk.",
      "Governance recommendations: which publishers need compliance review based on transaction quality signals?",
    ],
    voice: "Frame transaction quality as a financial controls issue, not just a performance metric. Unapproved GMV is real money at risk.",
  },
  6: {
    title: "Tier Management",
    description: "Governance framework for partner tiering, differentiated treatment, investment levels, and graduation criteria.",
    scr: {
      situation: "Describe the current implicit or explicit tiering in the program based on GMV concentration and publisher type distribution.",
      complication: "Without a formal tier structure, high-value partners receive the same support as low-value partners — misallocating manager time, commission budget, and enablement resources.",
      resolution: "Define a formal 3-4 tier structure (Strategic/Gold/Silver/Activated) with specific criteria, differentiated benefits, and graduation rules.",
    },
    required: [
      "Tier criteria: define GMV thresholds and activity requirements for each tier based on actual data distribution.",
      "Partner count and GMV coverage at each proposed tier.",
      "Differentiated benefits framework: Strategic (dedicated manager, co-investment, exclusives), Gold (quarterly reviews, priority support), Silver (automated comms, self-serve tools), Activated (onboarding only).",
      "Investment allocation: recommend specific % of total commission budget to allocate per tier.",
      "Graduation path: what does a Silver partner need to achieve to move to Gold?",
    ],
    voice: "Tier structure is an operating model decision, not an arbitrary VIP program. Every tier must have distinct economics and support cost.",
  },
  7: {
    title: "Action Plan Recommendations",
    description: "SMART, sequenced action roadmap: prioritized interventions with owners, deadlines, KPI targets, and strategic rationale.",
    scr: {
      situation: "Synthesize the key findings from all performance modules into a single prioritized action set.",
      complication: "Without clear prioritization and ownership, affiliate programs improve slowly. The 3-5 highest-impact interventions — if executed well — can compound into outsized results in a 90-day window.",
      resolution: "A three-horizon action plan (30/60/90 days) with specific, measurable actions. Each action must name an owner, a KPI target, and a specific completion criterion.",
    },
    required: [
      "Maximum 8 actions total (avoid boiling the ocean). Prioritize by expected GMV or ROI impact.",
      "Each action must be SMART: Specific (what exactly), Measurable (what KPI), Achievable (based on current data), Relevant (which finding it addresses), Time-bound (30/60/90 day).",
      "Owner role for each action: Affiliate Manager, Content Lead, Partnerships, Analytics, Finance.",
      "For each action, state the 'definition of done': what observable outcome confirms it was completed?",
      "Sequence actions with dependencies: some actions unlock others. Show the critical path.",
    ],
    voice: "Think like a management consultant writing a 'next steps' slide for a CEO. Specific, owned, sequenced. Not generic 'improve activation'.",
  },
  8: {
    title: "Timeline & Roadmap",
    description: "Phase-gated implementation roadmap with milestones, dependencies, and resource requirements through 90-day and 6-month horizons.",
    scr: {
      situation: "Frame the program improvement journey: where the program is today, where it needs to be, and the key waypoints.",
      complication: "Affiliate program improvements take 60-90 days to show in data due to attribution windows, partner onboarding cycles, and content production timelines. Sequencing matters.",
      resolution: "A milestone-based roadmap that accounts for dependencies, internal resource constraints, and partner-side lead times.",
    },
    required: [
      "Phase 1 (0-30 days): Quick wins — activation outreach, efficiency cleanup, tier communication.",
      "Phase 2 (30-60 days): Structural improvements — recruitment pipeline, content enablement, deal structure reform.",
      "Phase 3 (60-90 days): Scale and measure — new partners onboarded, early results visible, governance tightened.",
      "Dependencies: which actions must complete before others can start?",
      "Resource requirements: FTE estimate for each phase, external support needs.",
      "Success metrics: what specific numbers confirm the roadmap is on track at each phase gate?",
    ],
    voice: "A roadmap without dependencies and resource estimates is a wish list. Make this operationally credible.",
  },
  9: {
    title: "Data Quality Assessment",
    description: "Transparent audit of data completeness, field coverage, analytical confidence, and how gaps should affect decision-making.",
    scr: {
      situation: "State which data fields are present and complete, which are partial, and which are missing entirely.",
      complication: "Analytical conclusions are only as reliable as the underlying data. Partial approval breakdown, missing publisher type, or low order volume create specific blind spots.",
      resolution: "Confidence framework: which insights can be trusted at high confidence, which are directional, which should not be acted upon without better data.",
    },
    required: [
      "Field-by-field completeness assessment: present/partial/missing for each standard affiliate metric.",
      "Impact statement for each gap: 'Missing approval breakdown means we cannot diagnose decline root cause.'",
      "Confidence levels for each module: High (full data), Medium (partial data), Low (inference only).",
      "Data improvement recommendations: what data should be exported or captured next cycle?",
      "Decision guardrails: which actions should NOT be taken based on current data quality?",
    ],
    voice: "Intellectual honesty here builds trust. Do not oversell confidence in conclusions where data is incomplete.",
  },
  10: {
    title: "Appendix — Methodology",
    description: "Technical definitions, calculation methods, data sourcing, and the assumptions underpinning every metric and judgment.",
    scr: {
      situation: "Document the analytical engine: how each KPI was calculated, what data sources were used, and what assumptions were made.",
      complication: "Without methodology transparency, stakeholders cannot reproduce, challenge, or update the analysis — reducing its institutional value.",
      resolution: "A clear, auditable record of every formula, threshold, and judgment call in the report.",
    },
    required: [
      "Definition and formula for each KPI: active ratio, top-10 concentration, publishers-to-50%, approval rate, GMV/active publisher.",
      "Data field mapping: which source CSV columns map to which analytical fields.",
      "Threshold definitions and sources: where do benchmark values come from?",
      "Research methodology: how brand and market research was conducted, what sources were used.",
      "Limitations and assumptions: what would change these conclusions if the assumption were wrong?",
    ],
    voice: "Write for a data analyst who needs to reproduce this work six months from now.",
  },
  11: {
    title: "Category Landscape",
    description: "Market context for the brand's category: demand signals, seasonality, merchandising norms, and affiliate fit.",
    scr: {
      situation: "Define the category the brand competes in, its size signals, and the primary demand drivers.",
      complication: "Affiliate programs that ignore category dynamics allocate partners incorrectly — too many coupon partners in a content-led category, or vice versa.",
      resolution: "Category-specific affiliate strategy: which partner types win in this category, what content formats convert, and when to drive volume.",
    },
    required: [
      "Category name, primary demand signal, and trend direction (growing, stable, declining).",
      "Seasonality calendar: peak conversion windows with affiliate implications (Q4, product launches, etc.).",
      "Merchandising norms: how products in this category are typically sold through affiliates.",
      "Affiliate fit assessment: content vs deal vs comparison vs social — which mix is right for this category?",
      "Category-specific content opportunities: what review, comparison, or tutorial content drives conversion here?",
    ],
    voice: "Ground every claim in the supplied research evidence. Do not fabricate market size or trend data.",
  },
  12: {
    title: "Competitor Benchmark",
    description: "Competitive positioning matrix with affiliate program signals, pricing cues, and strategic differentiation implications.",
    scr: {
      situation: "Identify the top 3-4 direct competitors, their positioning, and visible affiliate program signals.",
      complication: "If competitors are running more sophisticated affiliate programs (higher commissions, better creative assets, more partner types), the brand risks losing mid-tier partners to competitive programs.",
      resolution: "Competitive response playbook: where to match, where to differentiate, and where to lead.",
    },
    required: [
      "Competitor snapshot table: name, domain, positioning, product focus, pricing tier, affiliate signals.",
      "Comparative assessment: where is the brand stronger, where is it weaker, where is the opportunity to differentiate?",
      "Partner poaching risk: are any top affiliates likely also running competitor programs?",
      "Commission competitiveness signal: if commission rate data is available, benchmark against competitors.",
      "3 specific competitive differentiation moves for the affiliate program.",
    ],
    voice: "Frame competitive analysis as actionable intelligence, not just a table of facts. 'Because Competitor X does Y, we should do Z' is the right pattern.",
  },
  13: {
    title: "Keyword Opportunity Map",
    description: "Search-intent cluster analysis for affiliate content strategy, creator brief development, and search capture.",
    scr: {
      situation: "Map the keyword landscape: branded, category, competitor comparison, and high-intent purchase modifier clusters.",
      complication: "If affiliates are not covering the right keyword themes, the program misses acquisition intent at the top and bottom of the funnel.",
      resolution: "Keyword-to-affiliate-action matrix: which clusters need content partners, which need comparison pages, which need coupon/deal coverage.",
    },
    required: [
      "4+ keyword clusters with theme, sample keywords, search intent, and affiliate angle for each.",
      "Volume/priority assessment: which clusters represent the highest affiliate conversion opportunity?",
      "Content gap analysis: which high-intent clusters have no current affiliate coverage?",
      "Creator brief direction: specific content angles that would capture primary keyword themes.",
      "Branded vs non-branded split: what % of affiliate traffic is captured through branded vs category search?",
    ],
    voice: "Keywords are a proxy for buyer intent. Frame each cluster as 'a buyer who wants X is looking for Y content from Z type of affiliate.'",
  },
  14: {
    title: "Brand Positioning & Creative Hooks",
    description: "Brand narrative analysis, key message frameworks, and partner-ready creative hooks for content and landing pages.",
    scr: {
      situation: "Summarize the brand's core positioning, hero products, proof points, and existing creator language.",
      complication: "If affiliate partners do not have a clear, compelling, brand-aligned creative brief, they will either produce generic content or avoid the brand entirely.",
      resolution: "A turnkey creative brief framework that any partner can use: hero messages, product hooks, proof points, content formats, and landing page requirements.",
    },
    required: [
      "Brand essence: 1-2 sentences that capture the core positioning a partner can use in content.",
      "3-5 creator hooks: specific angles or story frames that would resonate with each publisher type.",
      "Hero product content playbook: for each hero SKU, the right content format and conversion trigger.",
      "Landing page requirements: what existing product pages are affiliate-ready, what needs to be built.",
      "Trust and proof elements: reviews, awards, certifications that partners can reference.",
    ],
    voice: "Write as if you are the brand director briefing a creator agency. Make every hook specific and ownable.",
  },
  15: {
    title: "Research Sources",
    description: "Annotated bibliography of all citations, confidence assessment, research recency, and methodological limitations.",
    scr: {
      situation: "List all sources used in the report with full citation, the section they support, and their reliability rating.",
      complication: "Research quality varies. Some sources are primary (live website research), some are secondary (search results), some are inferred (model synthesis). Stakeholders must know which is which.",
      resolution: "A confidence-weighted source registry that helps readers know when to trust conclusions and when to verify independently.",
    },
    required: [
      "Full citation for every source: ID, title, URL, date accessed, relevance note.",
      "Source type classification: Primary (direct website scrape), Secondary (search result), Inferred (AI synthesis).",
      "Confidence rating per source: High (verified live), Medium (search result), Low (inferred/fallback).",
      "Recency note: is market research fresh enough to be actionable? Flag anything older than 6 months.",
      "Limitations statement: what the research cannot answer, and what additional research would improve confidence.",
    ],
    voice: "A source registry is a professional obligation, not a formality. Treat it as such.",
  },
};
