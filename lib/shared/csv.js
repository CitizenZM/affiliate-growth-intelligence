...
export function normalizeFieldMapping(headers = []) {
  const knownFields = {
    ...
    clicks: ["clicks", "num_clicks", "click_count", "all_clicks", "total_clicks"],
    orders: ["orders", "num_orders", "conversions", "actions", "transactions", "total_transactions", "approved_transactions"],
  ...
  headers.forEach((header) => {
    ...
    if (normalized === "total cost" && normalizedHeaders.has("action cost")) return;
    if (normalized === "approved_transactions" && normalizedHeaders.has("total_transactions")) return;
    const match = Object.entries(knownFields).find(([, values]) => values.includes(normalized));
    if (match) {
      if (!Object.values(mapping).includes(match[0])) {
