...
export const DEFAULT_SECTION_IDS = Array.from({ length: 16 }, (_, index) => index);
...
export function markdownFromSections(dataset, sections, metrics) {
  ...
  [...sections]
    ...
    .forEach((section) => {
      ...
      if (Array.isArray(section.summary_cards) && section.summary_cards.length) {
        parts.push("### Summary Cards");
        section.summary_cards.forEach((card) => {
          parts.push(`- ${card.label}: ${card.value}${card.insight ? ` — ${card.insight}` : ""}`);
        });
        parts.push("");
      }
      if (section.content_md) parts.push(section.content_md, "");
      if (Array.isArray(section.content_blocks) && section.content_blocks.length) {
        section.content_blocks.forEach((block) => {
          if (block.title) parts.push(`### ${block.title}`);
          if (block.body) parts.push(block.body);
          if (Array.isArray(block.items) && block.items.length) {
            const marker = block.type === "numbered_list" ? "1." : "-";
            block.items.forEach((item) => parts.push(`${marker} ${item}`));
          }
          if (block.meta) parts.push(`> ${block.meta}`);
          parts.push("");
        });
      }
      if (Array.isArray(section.tables) && section.tables.length) {
        section.tables.forEach((table) => {
          parts.push(`### ${table.title || "Table"}`);
      ...
      if (Array.isArray(section.citations) && section.citations.length) {
        ...
        section.citations.forEach((citation) => {
          parts.push(`- [${citation.id || "S"}] ${citation.title || citation.url} — ${citation.url}${citation.note ? ` (${citation.note})` : ""}`);
        });
        parts.push("");
      }
    });
